...
public static class VisitorTourSessionMapper
{
    public static VisitorTourSession Map(VisitorTourCard tour, TourSessionDto session, IReadOnlyList<VisitorPoi> pois)
    {
        var isCompleted = session.Status == TourSessionStatus.Completed || session.CurrentStopSequence >= tour.StopPoiIds.Count;
        if (isCompleted)
        {
            return new VisitorTourSession(
                TourId: tour.Id,

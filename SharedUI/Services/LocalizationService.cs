namespace FoodStreet.Client.Services
{
    public interface ILocalizationService
    {
        string this[string key] { get; }
        string CurrentLanguage { get; }
        event Action? OnLanguageChanged;
        void SetLanguage(string languageCode);
    }

    public class LocalizationService : ILocalizationService
    {
        public string CurrentLanguage { get; private set; } = "vi-VN";
        public event Action? OnLanguageChanged;

        private readonly Dictionary<string, Dictionary<string, string>> _translations = new()
        {
            ["vi-VN"] = new Dictionary<string, string>
            {
                // Login Page
                ["WelcomeBack"] = "Chào mừng trở lại",
                ["CreateAccount"] = "Tạo tài khoản",
                ["SignInSubtitle"] = "Đăng nhập để vào trang quản trị",
                ["RegisterSubtitle"] = "Đăng ký tài khoản quản trị mới",
                ["FullName"] = "Họ và tên",
                ["Email"] = "Email",
                ["Password"] = "Mật khẩu",
                ["ConfirmPassword"] = "Xác nhận mật khẩu",
                ["YourNamePlaceholder"] = "Nhập tên của bạn",
                ["EmailPlaceholder"] = "email",
                ["PasswordPlaceholder"] = "••••••••",
                ["SignInButton"] = "Đăng nhập",
                ["RegisterButton"] = "Tạo tài khoản",
                ["NoAccountRegister"] = "Chưa có tài khoản? Đăng ký",
                ["HaveAccountSignIn"] = "Đã có tài khoản? Đăng nhập",
                ["BackToHome"] = "Về trang chủ",
                ["LoginFailed"] = "Đăng nhập thất bại. Vui lòng kiểm tra email và mật khẩu.",
                ["PasswordsDoNotMatch"] = "Mật khẩu không khớp.",
                ["RegistrationSuccess"] = "Tạo tài khoản thành công! Bạn có thể đăng nhập ngay.",
                ["RegistrationFailed"] = "Đăng ký thất bại.",
                ["PleaseTryAgain"] = "Vui lòng thử lại.",
                ["ConnectionError"] = "Lỗi kết nối",
                ["EmailRequired"] = "Vui lòng nhập email",
                ["InvalidEmail"] = "Email không hợp lệ",
                ["PasswordRequired"] = "Vui lòng nhập mật khẩu",
                ["PasswordTooShort"] = "Mật khẩu phải có ít nhất 6 ký tự",

                // Dashboard
                ["Dashboard"] = "Bảng điều khiển",
                ["TotalFoods"] = "Tổng POI",
                ["TotalAudios"] = "Tổng audio",
                ["TotalUsers"] = "Tổng người dùng",
                ["QuickActions"] = "Thao tác nhanh",
                ["AddNewFood"] = "Thêm POI mới",
                ["ManageAudio"] = "Quản lý Audio",
                ["Translations"] = "Bản dịch",
                ["SystemInfo"] = "Thông tin hệ thống",
                ["ServerStatus"] = "Trạng thái máy chủ",
                ["Online"] = "Hoạt động",
                ["ClientVersion"] = "Phiên bản Client",

                // ManageFood
                ["ManageFoods"] = "Quản lý POI",
                ["AddFood"] = "Thêm POI",
                ["EditFood"] = "Sửa POI",
                ["DeleteFood"] = "Xóa POI",
                ["FoodName"] = "Tên POI",
                ["Description"] = "Mô tả",
                ["Latitude"] = "Vĩ độ",
                ["Longitude"] = "Kinh độ",
                ["Actions"] = "Thao tác",
                ["Save"] = "Lưu",
                ["Cancel"] = "Hủy",
                ["NoFoodsFound"] = "Chưa có POI nào",
                ["Loading"] = "Đang tải...",
                ["Price"] = "Giá",
                ["Location"] = "Vị trí",
                ["Edit"] = "Sửa",
                ["Delete"] = "Xóa",
                ["CreateNewFood"] = "Tạo POI mới",
                ["UploadAudio"] = "Tải lên Audio",
                ["SelectAudioFile"] = "Chọn file audio (MP3, WAV)",
                ["Uploading"] = "Đang tải lên...",
                ["ImageUrl"] = "Đường dẫn ảnh",
                ["Radius"] = "Bán kính (m)",
                ["Priority"] = "Độ ưu tiên",
                ["GeofenceRadius"] = "Bán kính kích hoạt geofence",
                ["HigherPriority"] = "Cao hơn = ưu tiên hơn",
                ["ClickAddFood"] = "Nhấn \"Thêm POI\" để tạo mục đầu tiên.",
                ["Name"] = "Tên",

                // Navigation
                ["Home"] = "Trang chủ",
                ["AdminPanel"] = "Quản trị",
                ["Settings"] = "Cài đặt",
                ["Logout"] = "Đăng xuất",
                ["StreetFoods"] = "Khám phá POI",
                ["AccessDenied"] = "Truy cập bị từ chối",
                ["PleaseLoginMessage"] = "Vui lòng đăng nhập để tiếp tục",
                ["Translations"] = "Bản dịch",
                
                // Admin Sidebar
                ["SectionMain"] = "CHÍNH",
                ["SectionContent"] = "NỘI DUNG",
                ["SectionAnalytics"] = "PHÂN TÍCH",
                ["NavDashboard"] = "Thống kê",
                ["NavManageFoods"] = "Quản lý POI",
                ["NavLocations"] = "Quản lý POI",
                ["NavTours"] = "Quản lý Tour",
                ["NavAudioFiles"] = "Tệp âm thanh",
                ["NavTranslations"] = "Bản dịch",
                ["NavQRCodes"] = "Mã QR",
                ["NavStats"] = "Thống kê",
                ["NavVisitors"] = "Lượt truy cập",
                ["NavHistory"] = "Lịch sử sử dụng",
                ["Search"] = "Tìm kiếm...",

                // Home Page
                ["AppTitle"] = "Thuyết Minh Ẩm Thực Vĩnh Khánh",
                ["HeroTitle"] = "Khám phá khu ẩm thực Vĩnh Khánh",
                ["HeroSubtitle"] = "Khám phá Vĩnh Khánh với GPS, QR code, tour trải nghiệm và thuyết minh đa ngôn ngữ",
                ["ExploreNow"] = "Khám Phá Ngay",
                ["FeaturesTitle"] = "Tính Năng Nổi Bật",
                ["FeatureGPS"] = "Định Vị GPS",
                ["FeatureGPSDesc"] = "Tự động gợi ý POI và trải nghiệm ẩm thực gần bạn",
                ["FeatureMultilingual"] = "Đa Ngôn Ngữ",
                ["FeatureMultilingualDesc"] = "Hỗ trợ Tiếng Việt, Anh, Nhật, Hàn, Trung",
                ["FeatureAudio"] = "Thuyết Minh Audio",
                ["FeatureAudioDesc"] = "Nghe thuyết minh cho từng POI và câu chuyện ẩm thực",
                ["FeatureMobile"] = "Tối Ưu Di Động",
                ["FeatureMobileDesc"] = "Hoạt động mượt mà trên mọi thiết bị",
                ["StatFoods"] = "POI",
                ["StatLanguages"] = "Ngôn Ngữ",
                ["StatAudios"] = "File Audio",
                ["CTATitle"] = "Sẵn sàng khám phá Vĩnh Khánh?",
                ["CTASubtitle"] = "Bắt đầu hành trình ẩm thực của bạn ngay hôm nay!",
                ["GetStarted"] = "Bắt Đầu Ngay"
            },
            ["en-US"] = new Dictionary<string, string>
            {
                // Login Page
                ["WelcomeBack"] = "Welcome Back",
                ["CreateAccount"] = "Create Account",
                ["SignInSubtitle"] = "Sign in to access admin panel",
                ["RegisterSubtitle"] = "Register a new admin account",
                ["FullName"] = "Full Name",
                ["Email"] = "Email",
                ["Password"] = "Password",
                ["ConfirmPassword"] = "Confirm Password",
                ["YourNamePlaceholder"] = "Your name",
                ["EmailPlaceholder"] = "email",
                ["PasswordPlaceholder"] = "••••••••",
                ["SignInButton"] = "Sign In",
                ["RegisterButton"] = "Create Account",
                ["NoAccountRegister"] = "Don't have an account? Register",
                ["HaveAccountSignIn"] = "Already have an account? Sign In",
                ["BackToHome"] = "Back to Home",
                ["LoginFailed"] = "Login failed. Please check your email and password.",
                ["PasswordsDoNotMatch"] = "Passwords do not match.",
                ["RegistrationSuccess"] = "Account created successfully! You can now sign in.",
                ["RegistrationFailed"] = "Registration failed.",
                ["PleaseTryAgain"] = "Please try again.",
                ["ConnectionError"] = "Connection error",
                ["EmailRequired"] = "Please enter email",
                ["InvalidEmail"] = "Invalid email format",
                ["PasswordRequired"] = "Please enter password",
                ["PasswordTooShort"] = "Password must be at least 6 characters",

                // Dashboard
                ["Dashboard"] = "Dashboard",
                ["TotalFoods"] = "Total POIs",
                ["TotalAudios"] = "Total Audios",
                ["TotalUsers"] = "Total Users",
                ["QuickActions"] = "Quick Actions",
                ["AddNewFood"] = "Add New POI",
                ["ManageAudio"] = "Manage Audio",
                ["Translations"] = "Translations",
                ["SystemInfo"] = "System Info",
                ["ServerStatus"] = "Server Status",
                ["Online"] = "Online",
                ["ClientVersion"] = "Client Version",

                // ManageFood
                ["ManageFoods"] = "Manage POI",
                ["AddFood"] = "Add POI",
                ["EditFood"] = "Edit POI",
                ["DeleteFood"] = "Delete POI",
                ["FoodName"] = "POI Name",
                ["Description"] = "Description",
                ["Latitude"] = "Latitude",
                ["Longitude"] = "Longitude",
                ["Actions"] = "Actions",
                ["Save"] = "Save",
                ["Cancel"] = "Cancel",
                ["NoFoodsFound"] = "No POIs found",
                ["Loading"] = "Loading...",
                ["Price"] = "Price",
                ["Location"] = "Location",
                ["Edit"] = "Edit",
                ["Delete"] = "Delete",
                ["CreateNewFood"] = "Create New POI",
                ["UploadAudio"] = "Upload Audio",
                ["SelectAudioFile"] = "Select an audio file (MP3, WAV)",
                ["Uploading"] = "Uploading...",
                ["ImageUrl"] = "Image URL",
                ["Radius"] = "Radius (m)",
                ["Priority"] = "Priority",
                ["GeofenceRadius"] = "Geofence activation radius",
                ["HigherPriority"] = "Higher = more priority",
                ["ClickAddFood"] = "Click \"Add POI\" to create your first entry.",
                ["Name"] = "Name",

                // Navigation
                ["Home"] = "Home",
                ["AdminPanel"] = "Admin Panel",
                ["Settings"] = "Settings",
                ["Logout"] = "Logout",
                ["StreetFoods"] = "POI Explorer",
                ["AccessDenied"] = "Access Denied",
                ["PleaseLoginMessage"] = "Please login to continue",
                ["Translations"] = "Translations",
                
                // Admin Sidebar
                ["SectionMain"] = "MAIN",
                ["SectionContent"] = "CONTENT",
                ["SectionAnalytics"] = "ANALYTICS",
                ["NavDashboard"] = "Dashboard",
                ["NavManageFoods"] = "Manage POI",
                ["NavLocations"] = "Manage POI",
                ["NavTours"] = "Tours",
                ["NavAudioFiles"] = "Audio Files",
                ["NavTranslations"] = "Translations",
                ["NavQRCodes"] = "QR Codes",
                ["NavStats"] = "Statistics",
                ["NavVisitors"] = "Visitors",
                ["NavHistory"] = "Usage History",
                ["Search"] = "Search...",

                // Home Page
                ["AppTitle"] = "Vinh Khanh Food Guide",
                ["HeroTitle"] = "Discover Vinh Khanh",
                ["HeroSubtitle"] = "Explore the food street with GPS guidance, QR entry points, tours, and multilingual narration",
                ["ExploreNow"] = "Explore Now",
                ["FeaturesTitle"] = "Key Features",
                ["FeatureGPS"] = "GPS Location",
                ["FeatureGPSDesc"] = "Automatically discover nearby POIs and food experiences",
                ["FeatureMultilingual"] = "Multilingual",
                ["FeatureMultilingualDesc"] = "Support Vietnamese, English, Japanese, Korean, Chinese",
                ["FeatureAudio"] = "Audio Guide",
                ["FeatureAudioDesc"] = "Listen to narration for each POI and food story",
                ["FeatureMobile"] = "Mobile Optimized",
                ["FeatureMobileDesc"] = "Works smoothly on all devices",
                ["StatFoods"] = "POIs",
                ["StatLanguages"] = "Languages",
                ["StatAudios"] = "Audio Files",
                ["CTATitle"] = "Ready to Explore?",
                ["CTASubtitle"] = "Start your Vinh Khanh journey today!",
                ["GetStarted"] = "Get Started"
            }
        };

        public string this[string key]
        {
            get
            {
                if (_translations.TryGetValue(CurrentLanguage, out var langDict))
                {
                    if (langDict.TryGetValue(key, out var value))
                        return value;
                }
                // Fallback to English
                if (_translations["en-US"].TryGetValue(key, out var fallback))
                    return fallback;
                return key; // Return key if not found
            }
        }

        public void SetLanguage(string languageCode)
        {
            if (CurrentLanguage != languageCode && _translations.ContainsKey(languageCode))
            {
                CurrentLanguage = languageCode;
                OnLanguageChanged?.Invoke();
            }
        }
    }
}
